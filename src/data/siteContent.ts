...
const CONTENT_KEY = "qawmi_site_content";
...
export const getSiteContent = (): SiteContent => {
  ...
  try {
    ...
    if (stored) {
      ...
      // Always use latest defaults for boards & categories items to pick up code updates
      return {
        ...defaultSiteContent,
        ...parsed,
        categories: { ...defaultSiteContent.categories, ...(parsed.categories || {}), items: defaultSiteContent.categories.items },
        boards: { ...defaultSiteContent.boards, ...(parsed.boards || {}), items: defaultSiteContent.boards.items },
        featured: { ...defaultSiteContent.featured, ...(parsed.featured || {}) },
        footer: { ...defaultSiteContent.footer, ...(parsed.footer || {}) },
        pages: { ...defaultSiteContent.pages, ...(parsed.pages || {}), signup: { ...defaultSiteContent.pages.signup, ...(parsed.pages?.signup || {}) } },
