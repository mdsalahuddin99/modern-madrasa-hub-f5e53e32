...
import { Users, GraduationCap, Calendar, BookOpen, ChevronRight, Bell, ImageIcon, ArrowRight, CheckCircle2, MessageSquareQuote, Target } from "lucide-react";
import { cn, toBn } from "@/lib/utils";
import Image from "next/image";
import { optimizeCloudinaryUrl } from "@/lib/cloudinary";

export default function OverviewPageClient({ madrasa }: { madrasa: any }) {
  const pathPrefix = `/madrasas/${madrasa.slug}`;

  // Stats
  const stats = [
    { label: "প্রতিষ্ঠিত", value: madrasa.establishedYear ? toBn(madrasa.establishedYear) : "অজানা", icon: Calendar, color: "text-blue-500", bg: "bg-blue-500/10" },
    { label: "ক্যাটাগরি", value: madrasa.category?.name || "সাধারণ", icon: BookOpen, color: "text-purple-500", bg: "bg-purple-500/10" },
    { label: "মোট ছাত্র", value: madrasa.totalStudents ? toBn(madrasa.totalStudents) : "০+", icon: Users, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { label: "মোট শিক্ষক", value: madrasa.totalTeachers ? toBn(madrasa.totalTeachers) : "০+", icon: GraduationCap, color: "text-orange-500", bg: "bg-orange-500/10" },
  ];
    ...
    <div className="flex flex-col gap-10 lg:gap-14">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, idx) => (
          <motion.div
            ...
            className="flex flex-col items-center justify-center p-6 rounded-3xl bg-white dark:bg-card border border-border/40 shadow-sm hover:shadow-md transition-shadow group text-center"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-10">
          {/* About Snippet */}
          <div className="bg-white dark:bg-card rounded-3xl p-6 md:p-8 border border-border/40 shadow-sm relative overflow-hidden">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {coreFeatures.map((feature: string, idx: number) => (
                  <div key={idx} className="flex items-start gap-3 bg-white dark:bg-card p-4 rounded-2xl border border-border/40 shadow-sm">
          ...
          {galleryImages.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href={`${pathPrefix}/gallery`} className="text-sm font-semibold text-primary hover:underline flex items-center gap-1">
          ...
          {departments.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href={`${pathPrefix}/academic`} className="text-sm font-semibold text-primary hover:underline flex items-center gap-1">
                  সব দেখুন <ChevronRight className="w-4 h-4" />
                </Link>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {departments.slice(0, 4).map((dept: any, idx: number) => (
                  <Link href={`${pathPrefix}/academic/${dept.type.toLowerCase()}`} key={idx} className="block group">
                    <div className="bg-white dark:bg-card p-5 rounded-2xl border border-border/40 shadow-sm hover:shadow-md transition-all group-hover:border-primary/30">
          ...
          {topTeachers.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href={`${pathPrefix}/about#teachers`} className="text-sm font-semibold text-primary hover:underline flex items-center gap-1">
                  সব দেখুন <ChevronRight className="w-4 h-4" />
                </Link>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {topTeachers.map((teacher: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 bg-white dark:bg-card p-4 rounded-2xl border border-border/40 shadow-sm">
        ...
        <div className="space-y-6">
          ...
          {/* Notice Board */}
          <div className="bg-white dark:bg-card rounded-3xl border border-border/40 shadow-sm overflow-hidden">
