...
import {
  plans, getSubscriptions, saveSubscriptions, MadrasaSubscription, formatBDT, toBn,
} from "@/data/subscriptions";
import { getAllMadrasas } from "@/data/getMadrasaList";
...
const SubscriptionForm = ({ madrasaId, madrasaName, preselectedPlan, onSuccess }: SubscriptionFormProps) => {
  ...
  const allMadrasas = getAllMadrasas();
  ...
  const handleSubmit = () => {
    ...
    const madrasa = allMadrasas.find(m => m.id === selectedMadrasa);
    const sub: MadrasaSubscription = {
      id: crypto.randomUUID(),
      madrasaId: selectedMadrasa,
      madrasaName: madrasa?.name || "অজানা",
      planId: selectedPlan,
      status: "pending",
      paymentMethod: paymentMethod as MadrasaSubscription["paymentMethod"],
      transactionId,
      payerPhone,
    ...
    setSubmitted(true);
    toast({ title: "✅ পেমেন্ট আবেদন জমা হয়েছে", description: "এডমিন যাচাই করার পর সাবস্ক্রিপশন সক্রিয় হবে।" });
    onSuccess?.();
  };
