...
const CTASection = () => {
  ...
  return (
    <section id="cta" className="section-padding pb-10 sm:pb-16 relative overflow-hidden">
      {/* Background decoration — same style as SearchSection/StatsBar */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 -left-20 w-80 h-80 bg-emerald-deep/[0.04] rounded-full blur-[120px]" />
        <div className="absolute bottom-0 -right-20 w-96 h-96 bg-gold/[0.04] rounded-full blur-[150px]" />
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <h2 className="text-2xl sm:text-3xl md:text-[2.75rem] font-extrabold text-foreground tracking-tight leading-[1.15]">
              {cta.title}
              <br />
              <span className="text-gradient-gold">{cta.titleLine2}</span>
            </h2>
          ...
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-5 sm:gap-7">
            ...
            >
              <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-emerald-deep/40 via-gold/40 to-emerald-deep/40" />
              <div className="p-5 sm:p-8 md:p-10 space-y-6 sm:space-y-7">
                {/* Live counter */}
                <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-gold/10 text-gold border border-gold/15 text-xs sm:text-sm font-semibold">
                ...
                <AnimatePresence>
                  {showUrgency && (
                    ...
                    >
                      <div className="p-4 sm:p-5 rounded-2xl bg-emerald-deep/5 border border-emerald-deep/15">
                        <div className="flex items-center justify-between mb-2.5">
                          <div className="flex items-center gap-2 text-xs sm:text-sm font-semibold text-emerald-deep">
                            <Target className="w-4 h-4 text-gold" />
                            <span>সাপ্তাহিক লক্ষ্য: <span className="text-foreground font-extrabold">{toBn(60)}টি</span></span>
                          </div>
                          <span className="text-xs sm:text-sm font-bold text-gold">{toBn(weeklyCount)}/{toBn(60)}</span>
                        </div>
                        <div className="w-full h-3 rounded-full bg-emerald-deep/10 overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${(weeklyCount / 60) * 100}%` }}
                            transition={{ duration: 1.8, ease: "easeOut", delay: 0.2 }}
                            className="h-full rounded-full bg-gradient-to-r from-gold to-gold-light relative"
                          >
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                  {whyWait.map(({ icon: Icon, label }, i) => (
                    <motion.div
                      ...
                      className="flex items-start gap-2.5 p-3 sm:p-3.5 rounded-xl bg-emerald-deep/[0.04] border border-emerald-deep/10 hover:bg-emerald-deep/[0.07] hover:border-emerald-deep/20 transition-all group"
                    >
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gold/10 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {benefits.map(({ icon: Icon, label }, i) => (
                    <motion.div
                      ...
                      className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-emerald-deep/5 border border-emerald-deep/15 text-xs sm:text-sm font-semibold text-emerald-deep"
                    >
                  ...
                  <motion.div
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-gold/10 border border-gold/20 text-xs sm:text-sm font-semibold text-gold"
                  >
                ...
                >
                  <Button
                    ...
                    className={`relative h-14 sm:h-16 px-7 sm:px-10 rounded-2xl text-base sm:text-lg font-bold transition-all duration-500 gap-2.5 ${
                      hoverBtn
                        ? "bg-emerald-deep text-white shadow-xl shadow-emerald-deep/30 -translate-y-0.5"
            ...
            >
                ...
                <div className="space-y-2">
                  {recentActivity.map((item, i) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-emerald-deep/[0.03] transition-colors"
                    >
                      <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-gold/30 to-gold/10 flex items-center justify-center text-[10px] font-bold text-gold shrink-0">
                        {item.initials}
                      </div>
              ...
              <div className="rounded-2xl sm:rounded-3xl bg-card border border-border/30 shadow-sm p-5 sm:p-6 md:p-7">
                <AnimatePresence mode="wait">
                  ...
                  >
                    <Quote className="w-5 h-5 text-gold/30 mb-2.5" />
                    <p className="text-xs sm:text-sm text-foreground/80 leading-relaxed mb-4">
                      &ldquo;{reviewQuotes[activeQuote].text}&rdquo;
                    </p>
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-deep to-primary flex items-center justify-center text-white text-[10px] font-bold">
                  ...
                  {reviewQuotes.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all duration-400 ${
                        i === activeQuote ? "w-6 bg-gold" : "w-1.5 bg-border hover:bg-muted-foreground/40"
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <div
                    key={label}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-deep/[0.04] border border-emerald-deep/10 text-[10px] font-semibold text-muted-foreground"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 text-center">
              <div>
                <p className="text-xl sm:text-2xl font-extrabold text-foreground tabular-nums">
                  <span ref={mRef}>{toBn(madrasaCount)}</span><span className="text-gold">+</span>
                </p>
                <p className="text-xs text-muted-foreground font-medium">মোট নিবন্ধিত</p>
              </div>
              <div>
                <p className="text-xl sm:text-2xl font-extrabold text-foreground tabular-nums">
                  <span>১২,৫০০</span><span className="text-gold">+</span>
                </p>
                <p className="text-xs text-muted-foreground font-medium">শিক্ষার্থী</p>
              </div>
              <div>
                <p className="text-xl sm:text-2xl font-extrabold text-foreground tabular-nums">
                  <span>৬৪</span><span className="text-gold">টি</span>
                </p>
                <p className="text-xs text-muted-foreground font-medium">জেলা কভার</p>
              </div>
              <div>
                <p className="text-xl sm:text-2xl font-extrabold text-foreground tabular-nums">
                  <span className="text-emerald-deep">১২৭</span><span className="text-gold">%</span>
                </p>
