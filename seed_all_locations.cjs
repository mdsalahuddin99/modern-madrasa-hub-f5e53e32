...
const districtsByDivision = {
  ...
  "ময়মনসিংহ": ["ময়মনসিংহ", "জামালপুর", "শেরপুর", "নেত্রকোনা"], // Note: match the nameBn exact
};
...
async function main() {
  ...
  for (const div of divisions) {
    ...
    // Check if dist array exists, some keys might have different spacing or spelling
    const divKey = Object.keys(districtsByDivision).find(k => k === div.nameBn);
    if (!divKey) continue;
    
    const dists = districtsByDivision[divKey] || [];
    
    for (const distName of dists) {
      const distSlug = distName.replace(/\s+/g, '-').toLowerCase();
      const district = await prisma.district.upsert({
        where: { slug: distSlug },
        update: {},
        create: {
          nameBn: distName,
          nameEn: distName, // simple fallback
          slug: distSlug,
          divisionId: division.id
        }
      });
      
      const thanaKey = Object.keys(thanasByDistrict).find(k => k === distName);
      if (!thanaKey) continue;
      const ths = thanasByDistrict[thanaKey] || [];
      
      for (const thName of ths) {
        const thSlug = thName.replace(/\s+/g, '-').toLowerCase() + '-' + distSlug; // prevent duplicates across districts
        await prisma.thana.upsert({
          where: { slug: thSlug },
          update: {},
          create: {
            nameBn: thName,
            nameEn: thName, // simple fallback
            slug: thSlug,
